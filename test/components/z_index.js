describe('Z-Index in the isometric grid', function() {
  ...
  describe('for tiles', function() {
    ...
    it('z-index should be greater when x is greater and z is lower (same y)', function() {
      tile1.tile(2, 1, 0, floorType, levelInfo);
      tile2.tile(1, 1, 4, floorType, levelInfo);
      expect(tile1.attr('z')).to.be.above(tile2.attr('z'));
    });
    ...
    it('z-index should be symmetrical with respect to x and y axis', function() {
      tile1.tile(2, 1, 4, floorType, levelInfo);
      tile2.tile(1, 2, 4, floorType, levelInfo);
      expect(tile1.attr('z')).to.equal(tile2.attr('z'));

      tile1.tile(1, 2, 4, floorType, levelInfo);
      tile2.tile(2, 1, 4, floorType, levelInfo);
      expect(tile1.attr('z')).to.equal(tile2.attr('z'));

      tile1.tile(2, 1, 4, floorType, levelInfo);
      tile2.tile(1, 2, 3, floorType, levelInfo);
      expect(tile1.attr('z')).to.be.above(tile2.attr('z'));

      tile1.tile(1, 2, 4, floorType, levelInfo);
      tile2.tile(2, 1, 3, floorType, levelInfo);
      expect(tile1.attr('z')).to.be.above(tile2.attr('z'));

      tile1.tile(2, 1, 3, floorType, levelInfo);
      tile2.tile(1, 2, 4, floorType, levelInfo);
      expect(tile1.attr('z')).to.be.below(tile2.attr('z'));

      tile1.tile(1, 2, 3, floorType, levelInfo);
      tile2.tile(2, 1, 4, floorType, levelInfo);
      expect(tile1.attr('z')).to.be.below(tile2.attr('z'));
    });
  ...
  describe('for the standing bot', function() {
    ...
    it('z-index of the standing bot should be greater than z-index of the tile it is standing on', function() {
      tile1.tile(1, 1, 0, floorType, levelInfo);
      bot.bot({x: 1, y: 1, z: 1});
  ...
  describe('for the bot moving in the plane', function() {
    ...
    function testMovingBot(heights, botPos, step, above, below, done) {
      tiles[0].tile(0, 0, heights[0], floorType, levelInfo);
      tiles[1].tile(1, 0, heights[1], floorType, levelInfo);
      tiles[2].tile(0, 1, heights[2], floorType, levelInfo);
      tiles[3].tile(1, 1, heights[3], floorType, levelInfo);
      tiles[4].tile(0, 2, heights[4], floorType, levelInfo);
      tiles[5].tile(1, 2, heights[5], floorType, levelInfo);
